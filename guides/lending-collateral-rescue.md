...
| :--- | :--- | :--- | :--- |
| `"Flash loan request failed"` | A failure occurred during the flash loan operation. | One of the internal flash loan steps (borrowing, repaying debt to the lending pool, withdrawing collateral, or DEX swapping) was rejected on-chain. | Review your position on the lending market to ensure collateral is unlocked, check swap slippage settings, or inspect the transaction trace on a block explorer to see which step reverted. |
