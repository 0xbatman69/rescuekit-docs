...
| :--- | :--- | :--- | :--- |
| `"Flash loan request failed"` | A failure occurred during the flash loan operation. | The contract wraps the entire multi-step rescue (borrowing debt, repaying debt to the lending market, withdrawing collateral, DEX swapping, and balance verification) inside a single callback. If any step fails—whether the pool lacks liquidity, debt repayment is rejected, collateral is locked, Uniswap slippage is exceeded, or residual balance is short—the contract catches the revert and emits this generic error. | Because the contract aggregates all callback failures under this error, the exact root cause cannot be known from the revert string alone. Inspect the transaction or simulation trace manually (e.g. via Tenderly, Phalcon, or block explorer simulation) to see which internal step reverted. |
