...
- **Fee-First Settlement Architecture (Anti-Exploit Protection)**: Protocol fees are settled atomically *before* the remaining balance is dispatched to the safe destination. This design prevents malicious tokens, griefers, or bad actors from blacklisting the treasury address or engineering revert traps to siphon assets fee-free. Because of this, users must always supply a clean, unencumbered recovery wallet—if `safeDestination` rejects the transfer, the protocol fee remains collected, and the remaining 85% (or 91%) stays behind in the compromised wallet.
